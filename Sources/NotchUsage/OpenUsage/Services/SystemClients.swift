...
struct SecurityKeychainAccessor: KeychainAccessing {
    ...
    // OpenNotch: the value rides in `security`'s arguments for the milliseconds it runs. These items
    // trust /usr/bin/security, so any process of this user can already read them through it; writing
    // in-process instead would change their access lists and lock the tools that own them out.
    func writeGenericPassword(service: String, value: String) throws {

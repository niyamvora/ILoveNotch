...
public final class NotesFeature: NotchFeature {
    ...
    /// Writes every unsaved note now, renaming its file when its title changed; a note emptied out
    /// is deleted rather than kept blank.
    func flush() {
        pendingSave?.cancel()
        ...
        for id in unsaved {
            ...
            var name = freeName(for: note.title, in: directory, keeping: note.fileName)
